...
Complete debugging demonstration for ESP32-C6 with all EAB features enabled.
...
1. **Test with EAB regression framework:**
   ```bash
   eabctl regression --test tests/hw/esp32c6_debug_full.yaml
   ```

2. **Create test YAML** (see `tests/hw/esp32c6_debug_full.yaml`)
