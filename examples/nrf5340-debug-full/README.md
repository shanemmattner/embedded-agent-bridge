...
Complete debugging demonstration for nRF5340 (ARM Cortex-M33) with all EAB Zephyr features enabled.
...
Open https://ui.perfetto.dev and load `/tmp/nrf5340-trace.json` to see:
...
### RTT Issues
- **No output:** Check J-Link connection, ensure `--device NRF5340_XXAA_APP`
...
### nRF5340 vs Other Platforms

| Feature | nRF5340 | ESP32-C6 | ESP32-S3 |
|---------|---------|----------|----------|
...
1. **Test with EAB regression framework:**
   ```bash
   eabctl regression --test tests/hw/nrf5340_debug_full.yaml
   ```

2. **Create test YAML** (see `tests/hw/nrf5340_debug_full.yaml`)

3. **Verify CTF → Perfetto pipeline** works end-to-end

4. **Try `babeltrace` directly:**
   ```bash
   babeltrace /tmp/nrf5340-trace.rttbin
   ```
...
- [nRF5340 DK User Guide](https://infocenter.nordicsemi.com/topic/ug_nrf5340_dk/UG/nrf5340_DK/intro.html)
