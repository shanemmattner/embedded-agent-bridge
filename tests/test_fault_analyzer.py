...
def _make_probe(gdb_port=2331, gdb_running=True, gdb_error=None):
    """Create a mocked DebugProbe."""
    probe = MagicMock(spec=DebugProbe)
    probe.gdb_port = gdb_port
    probe.start_gdb_server.return_value = GDBServerStatus(
        running=gdb_running, pid=12345, port=gdb_port, last_error=gdb_error
    )
    return probe


class TestAnalyzeFaultIntegration:
    """Test full pipeline with mocked DebugProbe and GDB subprocess."""
    ...
    def test_full_pipeline(self, mock_gdb_batch, tmp_path):
        ...
        report = analyze_fault(probe, device="NRF5340_XXAA_APP")
    ...
    @patch("eab.fault_analyzer.run_gdb_batch")
    def test_full_pipeline_with_custom_port(self, mock_gdb_batch, tmp_path):
        """analyze_fault() should work with a DebugProbe with custom port."""
        probe = _make_probe(gdb_port=3333)
    ...
    def test_rtt_stop_and_restart(self, mock_gdb_batch, tmp_path):
        ...
        rtt_bridge = MagicMock()
        rtt_status = MagicMock()
        rtt_status.running = True
        rtt_bridge.rtt_status.return_value = rtt_status
        ...
        report = analyze_fault(probe, device="NRF5340_XXAA_APP", restart_rtt=True, rtt_bridge=rtt_bridge)

        rtt_bridge.stop_rtt.assert_called_once()
        rtt_bridge.start_rtt.assert_called_once_with(device="NRF5340_XXAA_APP")
    ...
    def test_gdb_server_failure(self, mock_gdb_batch, tmp_path):
        ...
        probe = _make_probe(gdb_running=False, gdb_error="JLinkGDBServer not found")
    ...
    def test_custom_decoder_override(self, mock_gdb_batch):
        ...
        report = analyze_fault(probe, device="NRF5340_XXAA_APP", decoder=decoder)
